...
//       }
//     } 
//     // If 'did not', any value is fine, so no else block needed.
...
//     if (year !== currentYear) {
//       return { isError: true, message: `The year for '' must be the current year ().` };
//     }
//     return { isMatch: true };
//   };
...
//     const researchField = "My research did did not reveal any prior sales or transfers of the subject property for the three years prior to the effective date of this appraisal";
//     let researchValue = '';
//     // Robust lookup for the checkbox field
//     const keys = Object.keys(data);
//     const matchKey = keys.find(k => k.includes("My research did did not reveal any prior sales or transfers of the subject property"));
//     if (matchKey) researchValue = data[matchKey];
//     else if (data.PRIOR_SALE_HISTORY) {
//         const subKeys = Object.keys(data.PRIOR_SALE_HISTORY);
//         const subMatch = subKeys.find(k => k.includes("My research did did not reveal any prior sales or transfers of the subject property"));
//         if (subMatch) researchValue = data.PRIOR_SALE_HISTORY[subMatch];
//     }

//     if (String(researchValue).toLowerCase().includes('did not')) {
//         return { isMatch: true };
//     }
...
//     let researchValue = '';
//     const keys = Object.keys(data);
//     const matchKey = keys.find(k => k.includes("My research did did not reveal any prior sales or transfers of the comparable sales"));
//     if (matchKey) researchValue = data[matchKey];
//     else if (data.PRIOR_SALE_HISTORY) {
//         const subKeys = Object.keys(data.PRIOR_SALE_HISTORY);
//         const subMatch = subKeys.find(k => k.includes("My research did did not reveal any prior sales or transfers of the comparable sales"));
//         if (subMatch) researchValue = data.PRIOR_SALE_HISTORY[subMatch];
//     }
...
//     // Handle potential "s01/24;c12/23" format in sale date
//     let actualSaleDate = new Date(saleDateStr);
//     if (isNaN(actualSaleDate.getTime())) {
//         const sMatch = saleDateStr.match(/s(\d{1,2}\/\d{2,4})/i);
//         if (sMatch) {
//             actualSaleDate = new Date(sMatch[1]);
//         }
//     }

//     if (isNaN(priorSaleDate.getTime()) || isNaN(actualSaleDate.getTime())) return null;
    
//     const oneYear = 365 * 24 * 60 * 60 * 1000;
//     const diffTime = actualSaleDate - priorSaleDate;

//     if (priorSaleDate > actualSaleDate) {
//         return { isError: true, message: `Comparable's prior sale date () cannot be after its sale date.` };
//     }
